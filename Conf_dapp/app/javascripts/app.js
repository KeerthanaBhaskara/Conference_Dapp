...
return getBalance(conference.address);}).then(
function (balance) {
...
var self = this;
Conference.deployed().then(function (instance) {conference = instance;
...
function () {
return conference.numRegistrants.call();}).then(
function (num) {
$("#numRegistrants").html(num.toNumber());
return conference.registrantsPaid.call(buyerAddress);
}).then(
...
conference.refundTicket(buyerAddress,ticketPrice, {
from : accounts[0]
}).then(
function () {
return conference.numRegistrants.call();}).then(
...
var buyerAddress = $( "#mbuyerAddress" ).val();App.buyMultipleTickets(buyerAddress, web3.toWei(val), ticketcount);
});
